...
const res = await page.evaluate(async () => {
  ...
  const tiene = ['Use_Item', 'PickUp', 'Jump_Start', 'Jump_Land', 'Spawn_Ground'].map((n) => `${n}:${v.has(n)}`);
  // beber
  p.potions = 2; p.hp = 20; p.potionCooldown = 0;
...
console.log('al beber ->', res.alBeber, '| al recoger ->', res.alRecoger, '| al saltar ->', alSaltar);
