...
await page.evaluate(() => {
  ...
  const put = (m, dx, dz) => {
    ...
    m.z = p.z + dz;
    m.homeX = m.x;
    m.homeZ = m.z; // que no huyan por el leash
    m.px = m.x;
    m.pz = m.z;
  };
  const goblin = h.sim.mobs().find((m) => m.templateId === 'goblin');
  const giant = h.sim.mobs().find((m) => m.templateId === 'gigante');
  put(goblin, -2.4, -3.4); // al SUR del jugador: la camara los ve de cara
  put(giant, 2.2, -4.6);
  h.setCamera(Math.PI * 1.0, 0.3, 9);
  h.tickN(2);
});
await page.evaluate(
  () =>
...
await page.screenshot({ path: 'shots/30_goblin_gigante.png', timeout: 120000 });
console.log('shot: 30_goblin_gigante');
await browser.close();
server.kill();
process.exit(0);
