...
const ballesta = await page.evaluate(() => {
  ...
  h.setTimeOfDay(0.42); h.teleport(14, -82); h.setCamera(Math.PI * 0.15, 0.2, 6.5);
  p.yaw = Math.PI * 0.15 + Math.PI;
  const vivos = h.sim.mobs().filter((m) => m.alive);
  for (const m of vivos) { m.x = p.x + 300; m.z = p.z + 300; m.homeX = m.x; m.homeZ = m.z; }
  const mob = vivos[0];
  mob.x = p.x + Math.sin(p.yaw) * 9; mob.z = p.z + Math.cos(p.yaw) * 9; mob.hp = mob.maxHp * 20; mob.maxHp = mob.hp;
...
const voltereta = await page.evaluate(() => {
  ...
  return { esquivo: evs.some((e) => e.type === 'dodged'), anim: h.renderer.views.get(p.id)?.playing() };
});
await frames(2);
...
await page.evaluate(() => { window.__colosos.tickN(1, { swap: true }); window.__colosos.tickN(3); });
await frames(4);
const casco = await page.evaluate(() => {
  ...
  const cabezaVisible = () => v.meshes.filter((m) => /Helmet|Hat|Mask/.test(m.name)).map((m) => `${m.name}:${m.visible}`);
