...
const res = await page.evaluate(async () => {
  ...
  const nombres = ['swing','hit','hit_hard','crit','jump','land','land_hard','aggro','death_mob',
    'death_player','respawn','xp','levelup','mastery','block','ability','swap','loot_drop',
    'loot_pickup','bag_full','potion','talent'];
  a.unlock();
  for (const n of nombres) a.play(n);
  for (let i = 0; i < 6; i++) a.updateMusic(4, i % 2 === 0);
  await new Promise((r) => setTimeout(r, 600));
  const ctx = a.ctx ?? null;
  return { sonidos: nombres.length, estado: ctx ? ctx.state : 'sin contexto', ultimo: a.lastPlayed };
});
