...
const out = await page.evaluate(() => {
  ...
  const evs = h.sim.tick({ moveX: 0, moveZ: 0, jump: false, jumpHeld: false, attack: false, block: false, ability: false, sprint: false, swap: false });
  const types = evs.map((e) => e.type);
  for (const ev of evs) h.renderer.onSimEvent(ev);
  const el = document.querySelector('#toast');
  return { types, exists: !!el, cls: el?.className, text: el?.textContent, dropsLeft: h.sim.drops.length };
});
