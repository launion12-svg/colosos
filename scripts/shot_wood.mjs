...
await page.evaluate(() => {
  ...
  h.setCamera(Math.PI * 0.62, 0.3, 3.4); // primer plano de la espada
});
...
await page.screenshot({ path: 'shots/34_espada_madera.png', timeout: 120000 });
console.log('shot: 34_espada_madera');
await browser.close();
server.kill();
process.exit(0);
