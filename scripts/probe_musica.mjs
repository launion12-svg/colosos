...
const res = await page.evaluate(async () => {
  ...
  for (const el of [a.explorar, a.combate]) {
    el?.addEventListener('error', () =>
      window.__audioErr.push(`${el.src.split('/').pop()}: error ${el.error?.code}`),
    );
  }
  ...
  const traza = [];
  const paso = (dt, n) => {
    for (let i = 0; i < n; i++) a.updateMusic(dt);
  };
  ...
  a.setCombat(false);
  paso(0.25, 4);
  foto('1 s tras la pelea');
  paso(0.25, 8);
  foto('3 s tras la pelea');
  ...
  return {
    explorar: ficha(a.explorar),
    combate: ficha(a.combate),
    traza,
    audioErr: window.__audioErr,
    http,
    soportaMp3: new Audio().canPlayType('audio/mpeg'),
  };
});
